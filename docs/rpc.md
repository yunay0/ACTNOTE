...
> UI는 `meetings.notion_page_id` 가 `null` 이면 "Notion 동기화 중..." 스피너를 표시하다가 채워지면 링크 노출 권장. (Realtime subscribe 또는 5–10초 폴링)

**`/api/trigger-publish` Route Handler 예시:**

```ts
// app/api/trigger-publish/route.ts
...
이벤트 페이로드 상세는 `docs/events.md` 의 `meeting/publish` 항목 참조.
...
[프론트] supabase.rpc('create_invite', ...)            ← invite row 받음
       ...
       ↓
[Route Handler] 백엔드 Python 헬퍼 또는 동일 로직 수행
       ↓
       inngest.send('notification/email_send', { to, body_html, ... })
       ↓
[워커 send-email step] Resend.Emails.send → 수신자 메일함
```

본문 HTML 은 한국어 템플릿이라 백엔드의 `src/notifications.send_invite_email(...)` 또는 `src/email_notifier.render_invite_email(...)` 를 사용하는 게 가장 간단. Next.js 안에서 같은 본문을 직접 만들고 싶으면 [docs/notion-oauth.md](./notion-oauth.md) 의 callback 패턴처럼 `Buffer.from(html)` 으로 인라인 작성도 가능.

`/api/workspace/send-invite` Route Handler 예시:

```ts
// app/api/workspace/send-invite/route.ts
import { NextRequest, NextResponse } from "next/server";
import { Inngest } from "inngest";
import { createClient } from "@/lib/supabase/server";
...
export async function POST(req: NextRequest) {
  ...
  // 초대자 + 워크스페이스 정보 조회 (RLS가 멤버만 select 허용)
  ...
  // (간단판) HTML/text 를 직접 만들어 이벤트 발송. 운영에서는 백엔드 endpoint 위임 권장.
  const subject = `${inviterName}님이 ${workspaceName} 워크스페이스에 초대했습니다`;
  await inngest.send({
    name: "notification/email_send",
    data: {
      to: invite.invited_email,
      subject,
      body_html: `<p><b>${workspaceName}</b> 워크스페이스에 합류하시겠어요?</p>
                  <p><a href="${inviteLink}">초대 수락하기</a></p>`,
      body_text: `${subject}\n\n수락 링크:\n${inviteLink}`,
