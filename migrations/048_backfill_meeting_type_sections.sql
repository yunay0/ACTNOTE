-- 048: DRAFT-008-002 — 기존 회의의 유형별 신규 섹션 backfill
--
-- 배경:
--   migrations/044 가 meetings 에 신규 6개 컬럼(blockers/key_topics/key_decisions/
--   risks_and_issues/follow_up/key_points) 을 추가했다. 044 이전 회의들은 이 컬럼이
--   NULL 이고, 045 validate_meeting_for_publication 가 meeting_type 별 필수 섹션을
--   요구해 기존 회의의 재발행이 차단될 수 있다.
--
--   다행히 044 이전에도 ai_draft_notes JSONB 에 동일 키로 백업 저장되어 왔으므로
...
-- 운영 안내:
--   * 이 backfill 후에도 NULL 인 컬럼은 044 이전 회의가 해당 섹션을 처음부터 갖고
--     있지 않았던 경우다 (예: project_review 회의에 risks_and_issues 가 없었음).
--   * 사용자가 publish 하려면 (045 validate 에 따라) Edit Mode 에서 해당 섹션을 직접
